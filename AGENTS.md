...
5. `src/models/movie_encoder.py` encodes each movie's feature tokens.
6. `src/models/lett_me_pick.py` uses self-attention over rated context and cross-attention from query movies to that context to predict ratings.
...
- Collated movie tensors flatten the batch and movie-sequence dimensions. Model code restores the sequence dimension using `batch_size`.
...
- `src/models/movie_encoder.py`: Masked metadata-token encoder.
...
- Every function and method must have a brief docstring with `Args` and `Returns` sections.
...
- Model changes should cover tensor shapes, finite outputs or losses, and both `fusion` and `concat` score modes when affected.
